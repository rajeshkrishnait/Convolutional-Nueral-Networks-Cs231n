...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        xmean_diff=x-x_mean
        xMeanDiffSquare=xmean_diff**2
        xvar=np.mean(xMeanDiffSquare,axis=0)
...
def batchnorm_backward(dout, cache):
    ...
    # xhat=invstd*xmeandiff
    d_inv_std=cache['xmean_diff']*dxhat
    d_inv_std=d_inv_std.sum(axis=0)

    d_meandiff=cache['xstd_inv']*dxhat
    d_meandiff=d_meandiff.sum(axis=0)

    # xstd=1/xstd
    dxtd=(-1/cache['xstd']**2)*d_inv_std

    # xvar=np.sqrt(xvar+eps)
    dxvar = (1 / (2 * np.sqrt(cache['xvar'] + cache['eps']))) * dxtd
    
    d_xmean_diff_square = dxvar / N
    diff1=(2*cache['xmean_diff'])*d_xmean_diff_square
    diff2=cache['xstd_inv']*dxhat
    d_xmean_diff=diff1+diff2
    ...
    for index in range(N):
      ...
      for i in range(0,h_pad-FH+1,stride):
        for j in range(0,w_pad-FW+1,stride):
          x_col[:,neuron]=x_pad[index,:,i:i+FH,j:j+FW].reshape(C*FW*FH)
          neuron+=1
...
def conv_backward_naive(dout, cache):
    ...
    w_row=w.reshape(F,C*FH*FW)
    x_col = np.zeros((C*FH*FW, Hout*Wout))                   #[C*FH*FW x H'*W']

    for i in range(N):
      out_i=dout[i].reshape(F,Hout*Wout)
      w_out=w_row.T.dot(out_i)
      x_current=np.zeros((C,Hpad,Wpad))
      neuron=0
      for h in range(0,Hpad-FH+1,stride):
        for w in range(0,Wpad-FW+1,stride):
          x_current[:,h:h+FH,w:w+FW]+=w_out[:,neuron].reshape(C,FH,FW)
          x_col[:,neuron]=x_pad[i,:,h:h+FH,w:w+FW].reshape(C*FH*FW)
          neuron+=1
      dx[i]=x_current[:,pad:-pad,pad:-pad]
      dw+=out_i.dot(x_col.T).reshape(F,C,FH,FW)
      db+=out_i.sum(axis=1)
